...
class _buyCardState extends State<buyCard> {
  String deliverText = "Chọn phương thức thanh toán";
  String userNameShop;
  shippingInfor item;
    ...
    name.forEach((element) {
      ...
      widget.listCard.forEach((product) {
        if (product.shop.userName == element) {
          totalMoneyPayForCurrentShop +=
              product.amount * int.parse(product.price);
        }
      });
      ...
      var shop = {
        'shopName': element,
        'totalMoney': totalMoneyPayForCurrentShop,
        'feeShip': feeShip
      };
              ...
              child: Column(
                children: [
                  for (int i = 0; i < widget.listCard.length; i++)
                    listProducts(
                        shop: widget.listCard[i].shop,
                        productName: widget.listCard[i].nameProduct,
                        img: widget.listCard[i].img,
                        price: widget.listCard[i].price,
                        amount: widget.listCard[i].amount),
                ...
                if (item == null) {
                  ...
                  widget.listCard.forEach((element) {
                    PurchaseService().buyProducts(
                        ...
                        element.shop.uid,
                        element.amount,
                        element.amount * int.parse(element.price));
                    PurchaseService()
                        .deteleItemInCart(widget.user.uid, element.productID);
                  });
  ...
  listProducts(
      {User shop, String productName, String img, String price, int amount}) {
    return Column(
      children: [
        (shop.userName != userNameShop)
            ? choiceDeliver(shop.userName)
            : SizedBox(
                height: 0,
              ),
        Container(
          height: 35,
          decoration: BoxDecoration(color: Color(4294565598), boxShadow: [
            BoxShadow(
              color: Colors.grey,
            ),
          ]),
  ...
  paymentMethods() {
    return Column(
      children: [
        Row(
          children: [
            FlatButton.icon(
                      ...
                      builder: (BuildContext context) => AlertDialog(
                        ...
                        actions: <Widget>[
                          FlatButton(
                              onPressed: () {
                                setState(() {
                                  deliverText = "Thanh toán sau khi nhận hàng";
                                });
                                Navigator.pop(context, deliverText);
                              },
                              child: Row(
                                children: [
                          ...
                          FlatButton(
                              onPressed: () => Navigator.push(
                                  ...
                                  MaterialPageRoute(
                                      builder: (context) =>
                                          MomoWallet(widget.user, totalMoney))),
                              child: Row(
                                children: [
                ...
                label: Text(deliverText, style: TextStyle(fontSize: 18))),
            /*Row(
                children: [
        ...
        Padding(
          padding:
              const EdgeInsets.only(left: 15.0, top: 5, right: 15, bottom: 15),
          child: Column(
            children: [
              Row(
                children: [
                  ...
                  Spacer(),
                  Text(totalMoneyPayForProduct.toString() + 'd',
              ...
              Row(
                children: [
                  ...
                  Spacer(),
                  Text(totalMoneyPayForShip.toString() + 'd',
                      style: TextStyle(fontSize: 16, color: Colors.black54))
                ],
              ),
              Row(
                children: [
                  ...
                  Spacer(),
                  Text(totalMoney.toString() + 'đ',
                      style: TextStyle(fontSize: 18, color: Color(4294344335)))
                ],
              ),
            ],
          ),
